...
select_word_list = ["_"] * len(select_word)
...
print(select_word_list)

while count <= 4:
    while True:
        predict_word = input("含まれている文字を予想してください:")
    ...
    for i in select_word:
        if predict_word == i:
            select_word_list[str_count] = predict_word
            include = True
        str_count += 1

    if include:
        print(predict_word,"は含まれていました。")

    else:
        print(predict_word,"は含まれていませんでした。")
        count += 1
        print(hangman[count])
    ...
    if not "_" in select_word_list:
        print("正解です。おめでとうございます。")
        break

    elif count == 5:
        print("正解は",select_word,"でした。残念。")